...
This library is differs from the Rabbit library in the following ways:
1) Reads the Modbus address from an external variable rather than a #define
2) Responds to a broad cast command


Modbus_slave.LIB		Version 1.00
...
extern unsigned char MB_address;
	...
	_mbsReplyByte ( MB_address ); //	insert Slave Address
...
void msExec ( void )
{
	...
	switch ( wMSCmd ) {						//	Dispatch Handler
		...
		case	0x18	:							//	Read FIFO Queue
  		...
  		case	0x43	:							//	Load attenuator set value (same as 0x06)
      	nErr = mbsWriteReg ();
			break;
  		case	0x44	:							//	Load attenuator table (same as 0x10)
			nErr = mbsWriteRegs ();
			break;
...
{	auto int ByteCount;
	...
	if ( ByteCount > 0 )						// if valid byte count
   {
      ...
      } else if (acMSReply[0] == 0 && acMSReply[1] == 69) // event trigger
      {
			event_trigger();
      } else if (acMSReply[0] == 0 && (acMSReply[1] == 68 || acMSReply[1] == 67)) // attenuator value broadcast
      {
         memcpy ( acMSCmd, acMSReply, ByteCount ); // copy for msExec
         msExec ();											// execute the command
         ByteCount = pcMSReply-acMSReply; 			// byte count of response
